...
async def _persist_engagement(eng: Engagement) -> None:
    ...
    try:
        ...
        async with get_session() as session:
            ...
            existing = await repo.get(eng.engagement_id)
            if existing is None:
                await repo.create(eng)
            else:
                await repo.update_status(eng.engagement_id, eng.status.value)
...
async def get_engagement(engagement_id: UUID) -> dict:
    ...
    if not eng:
        raise HTTPException(404, "engagement not found")
    findings = _findings_by_eng.get(engagement_id, [])
    return {
        "engagement": eng,
...
async def list_findings(engagement_id: UUID) -> list[dict]:
    return [f.model_dump(mode="json") for f in _findings_by_eng.get(engagement_id, [])]
